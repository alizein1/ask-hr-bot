...
def show_dashboard(df, prompt):
    ...
    if "age" in prompt:
        ...
        df['Gender'].dropna().str.strip().str.upper().value_counts().plot(kind='bar')
        st.pyplot(plt.gcf())
        plt.clf()

    elif "job title" in prompt:
        st.subheader("Top 10 Job Titles")
        df['Job Title'].dropna().str.strip().value_counts().head(10).plot(kind='barh')
        st.pyplot(plt.gcf())
        plt.clf()

    elif "band" in prompt:
        st.subheader("Employees by Band")
        df['Band'].dropna().str.strip().value_counts().plot(kind='bar')
        st.pyplot(plt.gcf())
        plt.clf()

    elif "grade" in prompt:
        st.subheader("Employees by Grade")
        df['Grade'].dropna().str.strip().value_counts().plot(kind='bar')
        st.pyplot(plt.gcf())
        plt.clf()
