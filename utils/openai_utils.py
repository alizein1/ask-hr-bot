def ask_hr_excel_bot(df, prompt):
    ...
    # Detect entity if mentioned (e.g., Tawfeer, Capital Partners)
    ...
    # Detect nationality keyword
    nationality_match = re.search(r"how many (\w+) (nationals|people|employees)?", prompt_lower)
    if nationality_match:
        nationality = nationality_match.group(1).upper()
        count = filtered_df['Nationality'].str.upper().value_counts().get(nationality, 0)
        return f"There are {count} {nationality.title()} employees."
